; =============================================================================
;  AutoIt IconImage UDF (2014.3.19)
...
Func _IconImage_FromGdipBitmap($Gdibmp)
	...
	Do
		...
		DllStructSetData($Header, "biWidth", $Width)
		DllStructSetData($Header, "biHeight", $Height * 2)
		DllStructSetData($Header, "biSizeImage", $ColorSizeImage + $MaskSizeImage)
...
Func _IconImage_FromHandle($Handle)
	...
	Do
		...
		If @Error Or $ColorBmp = 0 Then ExitLoop
...
Func _IconImage_ToHandle($IconImage, $Width = Default, $Height = Default)
	...
	If $IconImage Then
		...
		If @Error Or $hIcon = 0 Then Return SetError(1, 0, 0)
		Return $hIcon
	EndIf
...
Func _IconImage_GetSize($IconImage)
	...
	ElseIf $IconImage Then
		...
		$Width = DllStructGetData($Header, "biWidth")
		$Height = DllStructGetData($Header, "biHeight") / 2
		$BitCount = DllStructGetData($Header, "biBitCount")
	Else
		$Error = 1
	EndIf
...
Func _IconImage_Overlay($IconImage1, $IconImage2)
	...
	Do
		...
		If @Error Or $hIcon3 = 0 Then ExitLoop
...
Func _IconImage_32BitAlphaToGdipBitmap($IconImage)
	...
	If DllStructGetData($Header, "biBitCount") <> 32 Then Return SetError(1, 0, 0)
	Local $Height = DllStructGetData($Header, "biHeight") / 2
	Local $Width = DllStructGetData($Header, "biWidth")
...
Func _IconImage_CreateMaskBmp($ColorBmp)
	...
	Do
		...
		DllStructSetData($Header, "biSize", 40)
		DllStructSetData($Header, "biWidth", $Width)
		DllStructSetData($Header, "biHeight", $Height)
		DllStructSetData($Header, "biPlanes", 1)
		DllStructSetData($Header, "biBitCount", 1)
		DllStructSetData($Header, "biSizeImage", $Stride * $Height)
