...
#cs
    ...
    Author:         Yashied
	Modified		MBee - Corrected _RDC_GetData() array return order
    Version:        1.1
    Requirements:   AutoIt v3.3.x.x, Developed/Tested on Windows XP Pro Service Pack 2 and Windows 7, 32 and 64-bit
    Uses:           None
...
; Author.........: Yashied
; Modified.......: MBee - Minor fix to return data array as specified in the documentation above - [n][0] = type, [n][1] = filename
...
Func _RDC_GetData($iID)
	...
	Do
		...
		_rdc_Inc($aData)
		$aData[$aData[0][0]][0] = DllStructGetData($tFNI, 2)
		$aData[$aData[0][0]][1] = DllStructGetData($tFNI, 4)
...
Func _RDC_OpenDll($sDLL = '')
	...
	If Not $sDLL Then
		If @AutoItX64 Then
			$sDLL = @ScriptDir & '\RDC_x64.dll'
		Else
			$sDLL = @ScriptDir & '\RDC.dll'
		EndIf
	EndIf
